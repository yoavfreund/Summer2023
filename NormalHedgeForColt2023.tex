...
\begin{minipage}[t]{6.4in}
  ...
  $\left\{\omega_1,\ldots,\omega_N\right\}$,
  $\omega_j \geq 0$, $\sum_j \omega_j =1$\\
  {\bf Algorithm} \\

  For $i=1,\ldots,T$
\begin{enumerate}
\item The learner chooses a distribution function $P_j^i$ over the actions
  $j \in \{1,\ldots,N\}$ such that $\sum_{j=1}^N P_j^i \omega_j=1$. We
  denote the vector of $N$ weights by $\Pvec{i}$
\item The adversary chooses an {\em instantaneous loss} for each
  of the $N$ actions: \\
  $l_j^t \in [-1,+1]$ for $j \in \{1,\ldots,N\}$. we defnote the
  vector of $N$ losses by $\Lvec{i}$
\item The {\em cumulative loss of action $j$} is
  $L^i_j = \sum_{s=1}^i l_j^s$.
\item The learner incurs an {\em instantaneous average loss} defined
  as $\ell^i = \sum_{s=1}^i \omega_jP^i_j l_j^i = \Pvec{i} \cdot \Lvec{i}$
...
\item The {\em cumulative regret} of the learner with respect to
  ...
  $\Rdist{i}$ represents the {\em distribution} of the regrets.
\end{enumerate}
