...
\item The adversary chooses an {\em instantaneous loss} for each
 ...
 \item The {\em cumulative loss of action $j$} is
    $L^i_j = \sum_{s=1}^i l_j^s$. we denote the
...
Potentials are auxiliary functions used to measure progress and
perform amortized analysis of the regret.  Typically~\cite{CesaBinachiLugosi}, potential
functions are positive, increasing and convex. We require somewhat
stronger conditions which we call strict positivity.
...
\subsubsection{Using potential to bound regret}
...
\subsubsection{Recusive definition of the potential}
We expand on previous work using potnetiaal functions in two ways:

In most applications, the potential assocciated with a specific action
is a function of the cumulative loss of the same action. In
NormalHedge~\cite{} the potential function also depends on the
iteration numer. 
...
A single action defines a path $S_\omega$ (as defined in
(\ref{eqn:path})). Fixing the strategies of the learner and the
adversary determines a distribution $\D$ over paths.  We describe two
