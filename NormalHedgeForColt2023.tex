...
\coltauthor{\Name{Yoav Freund} \Email{yfreund@ucsd.edu}}
\begin{document}
...
the weighted majority algorithm~\cite{LittlestoneWa94} to give {\em zero-order}
...
history that uniquely defines the future of the game. One can define
the state of the game at time $t$ as the vector consisting of the $N$
cumulative regrets: $(R_1^t,\ldots,R_N^t)$. Note also that the regret
(relative to the best action or a percentile) as well as the potential
are conserved under permutation of the regrets. Therefor, the a sufficient state is the {\em distribution} of the cumulative regrets.
...
the number of actions with that cumulative regret.

We extend the game to allow infinite, and uncountably infinite, sets
of actions. Formally, we define the set of actions as a probability
space $\Omega$. In this formalism an action is an element
$\omega \in \Omega$ and all of the quantities of interest - are random
variables~\footnote{formally, a measurable function over
  $\Omega$}. We define the state at time $t$ to be the distribution
(over the real line) of $R(\omega)$ and denote it by $\state(t)$. Note
that this formalism contains finite set of actions as a special case.

As argued above, all actions that have the same cumulative regret at
time $t$ are equivalent from ....
...
\end{minipage}}
\caption{The integer time potential game \label{fig:integerTimeGame}}
\end{figure}
