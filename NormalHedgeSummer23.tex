...
\begin{minipage}[t]{6.4in}
  {\bf Initialization:} $L^0_j=0$ for $j=1,\ldots,N$, $L_{\ell}^0 = 0$\\
  ~\\
For $i=0,1,\ldots$
\begin{enumerate}
...
We say that a function $f:R \to R$ is convex of degree $k$ if 
\begin{definition}
...
\begin{minipage}[t]{6.4in}
  ...
  and $\finalpot(\cdot)$, a positive, increasing and convex function.\\
  {\bf The adversary chooses} step size $s=1/k$ for some natural number $k$.
  $T=Sk^2$
~\\
...
\end{theorem}
\proof We show that the adversary can make choices that make the
...
\subsection{States}
We allow the randomized and history-dependent experts.
Specifically, we define $\omega_L,\omega_A$ to be random bit vectors
used as input to the Learner and the Adversary respectively. Define the history of the game to be   

The regret vector $\vec{R}$ can be seen as the {\em state} of the game
in the sense that it captures all of the relevant information from the
past.
\begin{theorem}
  Let $\vec{R}_0,\ldots \vec{R}^T$ be a sequence of regret vectors
  generated by playing the potential game.  Let
  $0\leq i < j < k \leq T$. Fix the vector $\vec{R}^j$ and fix the
  strategies of the learner and the adversary for iterations
  $j \leq l \leq k$. Then the vector $\vec{R}^k$ is independent of
  $\vec{R}^i$.
\end{theorem}
...
The potential game fixes the {\em final} potential $\pot_T$. We will
now show how to define a potential function for steps $T-1,T-2,\ldots 0$.

We start by fixing the strategies for the learner and the adversary. We show that
\begin{theorem}
  For any learner strategy $P$, adversarial strategy $Q$, total steps $T$ and final potential
  $\pot^T$ there exist a sequence of potentials $\pot^0,\pot^1,\ldots, \pot^{T-1}$ such that
  $$\vec{R}^T \odot \pot^T = \vec{R}^{T-1} \odot \pot^{T-1} = \cdots =
  \vec{R}^0 \odot \pot^0 $$
...
Upper potential for binomial learner.

Explanation why time is defined as it is.

\subsection{$s \to 0$ is preferred by adversary}

\subsection{$s \to 0$ converges to the min/max limit}

\subsection{$s \to 0$ Corresponds to Kolmogorov}

\subsection{Closed form sollution and stop anytime algorithms}
General first order bound for C4 potential functions.

\subsection{First order Bound for NormalHedge}

\subsection{Second order bound for NormalHedge}
