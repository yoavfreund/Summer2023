...
Technical results:
\begin{enumerate}
\item A min/max analysis of a finite horizon potential game for terminal potential with 4 strictly positive derivatives.  Resulting in a characterization of the min/max strategies as solutions of the heat equation.
  \item Using analytical solutions of the heat equation, we create a stop-anytime version of the learning algorithm.
\end{enumerate}
...
\item The learner chooses a distribution function $P_j^i$ over the experts
  $j \in \{1,\ldots,N\}$ \\
  such that $\sum_{j=1}^N P_j^i=1$. We
...
\item The adversary chooses an {\em instantaneous loss} for each
  ...
  vector of $T$ losses for expert $j$ by $\lvec{j}$
   vector of $T$ cumulative losses for expert $j$ by $\Lvec{j}$.
...
\item The {\em cumulative regret} of the learner with respect to
  expert $j$ is $\R_j^i = L_\ell^i -L_j^i $. \\
...
We now give an upper and lower bound on $\epsilon(V^T({\cal H}),R)$
\begin{theorem}

Let $V^T$ be the ${\cal H}$-cumulative variance 
for ${\cal H}$ defined in~(\ref{eqn:HNormalHedge})

then the Normal-Hedge algorithm, defined in Figure~\ref{fig:normalhedge2} satisfies:

\begin{equation} \label{eqn:epsilonUpperBound}
  \forall T,R,\;\; \epsilon(V^T({\cal H}),R) \leq \sqrt{V^T({\cal H})+1} \;\; \exp \paren{\frac{-R^2}{2(V^T({\cal H})+1)}}
\end{equation}
\end{theorem}
...
Given any sequence of numbers $0 \leq \nu^i \leq 1$ such that $\lim_{n \to \infty} \sum_{i=1}^n \nu_i = \infty$,
there exists an adversarial strategy such that $V^n({\cal I}) =  \sum_{i=1}^n \nu_i$ for any sequence of distributions ${\cal I}$,
and any learning algorithm.

For this adversarial strategy and for any $\delta>0, R>0$, there exists $\Theta(\delta,R)$
such that for all $T \geq \Theta(\delta,R)$:
  \begin{equation} \label{eqn:epsilonLowerBound}
    \epsilon(V^T({\cal H}),R) +\delta \geq
    \left(\frac{\sqrt{V^T({\cal H})}}{\R} -  \left( \frac{\sqrt{V^T({\cal H})}}{R} \right)^3 \right) \exp \paren{\frac{-R^2}{2V^T({\cal H})}}
  \end{equation}
\end{theorem}

If we define $a=\frac{\sqrt{V^T({\cal H})}}{\R}$ we can approximate the upper bound by 
\[
...
sets of experts.  We use a measure-theoretic framework. We introduce
...
counting experts.  We consider a {\em single trajectory of the
...
what follows the probabities do not encode dependence of the experts
on previous experts, i.e. the {\em strategies} used by the two
players. In fact, the min/max strategies we propose are deterministic.

\newcommand{\Ndist}{{\cal N}}

The probability space is defined as follows. Each expert corresponds
to a sequence of $T$ losses $\lvec{j} \in [-1,+1]^T$. The set of all
possible sequences is an outcome space $\Omega$. The {\em set} of
experts is represented by a distribution over $\Omega$. Specifically,
the $N$ experts setup defined in (Fig~\ref{fig:DTOL}) corresponds to a
distribution whose support is of size $N$ where the expert $\omega_j$
has probabilitiy $w_j$. We denote this distribution by $\Ndist$.

Each of the variables $P_j^i, l_j^i, L_j^i, R_j^i$ can be seen as a
mappings from experts $\omega \in \Omega$ to $\reals$.  In other
...
\begin{minipage}[t]{6.4in}
  {\bf Initialization:}\\
  A distribution $\D$ over a set of experts $\Omega$\\~\\
  {\bf Algorithm} \\
For $i=1,\ldots,T$
\begin{enumerate}
\item The learner chooses a density function $P^i(\omega)$ over $\Omega$
  such that $\E{\omega \sim \D}{P^i(\omega)}=1$
\item The adversary chooses an {\em instantaneous loss} for each expert:  $l^i: \Omega \to [-1,+1]$
 \item The {\em cumulative loss of expert $j$} is
...
\item The {\em cumulative loss of the learner} is
  $L_\ell^i = \sum_{s=1}^i \ell^s$
\item The {\em cumulative regret} of the learner with respect to
  expert $j$ is $\R^i(\omega) = L_\ell^i -L^i(\omega) $.
\end{enumerate}
\end{minipage}}
...
is finite. We need $2^T$ experts to describe each and every seuqence
generated by the random walk. Therefor by using $N=2^T$ we can use the
standard finite expert framework.

\subsection{Potential functions}
Potentials are auxiliary functions used for amortized analysis of
online learning algorithms. High potential is associated with experts
...
and regret for the general case in which the experts correspond to a
...
\begin{definition}{\bf Bound on percentile regret} Let $\epsilon>0$.
  If the set of experts is finite (size ($N$) then
...
\begin{definition}{\bf The final score} \label{def:averagePotential}
  The final score in the finite expert case is defined as
...
e.g.~\cite{cesa2006prediction}). One potential function that recieved
...
A single expert $\omega$ defines a sequence of $T$ losses $\Lvec{\omega}$ (as defined in
...
We are ready to prove theorem~\ref{thm:NHUpperBound}.
...
The time variable i$t_n$ is directly related to the cumulative
variance as follows.
...
We can use $V_n$  for the normal-hedge potential $\pot_{\mbox{\tiny NH$(\nu)$}}(t_n,\R)$ we get the Upper bound in theorem~\ref{thm:NHUpperBound}.
...
\proof {\bf of Lemma~(\ref{lemma:differences})}
